<script lang="ts">
	...
	const viewerSrc = $derived(
		debugUrl ? `/api/browser-viewer?url=${encodeURIComponent(debugUrl)}` : `/api/browser-viewer`
	);
</script>

<div
	class="relative flex h-full w-full flex-col overflow-hidden rounded-2xl border border-gray-700/50 bg-gray-900 shadow-xl"
>
	<!-- Glassmorphism header bar -->
	<div
		class="flex items-center gap-3 border-b border-gray-700/50 bg-gray-800/60 px-4 py-2.5 backdrop-blur-md"
	>
	...
	{:else if debugUrl}
		{#if !loaded}
			<div
				class="absolute inset-0 z-10 flex flex-col items-center justify-center gap-5 bg-gray-900"
			>
